        ...
        with engine.connect() as connection:
            pssl_base_data = pd.DataFrame(connection.execute(text(f'''
                select distinct
                    ...
                    case when bs."[CM] [CS] Cash" = 'NM' then null else bs."[CM] [CS] Cash" end as "current_unrestricted_cash",
                    case when bs."First Lien Debt" = 'NM' then null else bs."First Lien Debt" end as "current_gross_senior_debt",
                    case when bs."Total Debt" = 'NM' then null else bs."Total Debt" end as "current_gross_total_debt",
                    ...
                    case when bs."[CM] [R] LTM EBITDA" = 'NM' then null else bs."[CM] [R] LTM EBITDA" end as adjusted_ttm_ebiteda,
                    case when bs."[CM] [R] LTM Cash Interest" = 'NM' then null else bs."[CM] [R] LTM Cash Interest" end as current_cash_interest_expense,
