...
def soi_mapping(engine, extracted_base_data_info, master_comp_file_details, cash_file_details):
	...
	pbb."Closing LTM EBITDA" as initial_ttm_ebitda,
	pbb."Current LTM EBITDA" as current_ttm_ebitda,
	ch."As Of Date" as current_as_of_date,
	usbh."Maturity Date" as maturity_date,
	case when ss."[SI] Type of Rate" like 'Fixed Rate%' then 'Y' else 'N' end as fixed_rate,
	null as coupon_incl_pik_pikable,
	case when ss."[SI] LIBOR Floor" is not null then 'Y' else 'N' end as floor_obligation,
	...
	case when pbb."DIP Loans" = 0 then 'N' when pbb."DIP Loans" = 1 then 'Y' else null end as dip_loan,
	case when pbb."Obligations w/ Warrants attached" = 0 then 'N' when pbb."Obligations w/ Warrants attached" = 1 then 'Y' else null end as warrants_to_purchase_equity,
	case when pbb."Participations" = 0 then 'N' when pbb."Participations" = 1 then 'Y' else null end as participation,
	case when pbb."Convertible into Equity" = 0 then 'N' when pbb."Convertible into Equity" = 1 then 'Y' else null end as convertible_to_equity,
	case when pbb."Equity Security" = 0 then 'N' when pbb."Equity Security" = 1 then 'Y' else null end as equity_security,
	case when pbb."Subject of an Offer or Called for Redemption" = 0 then 'N' when pbb."Subject of an Offer or Called for Redemption" = 1 then 'Y' else null end as at_acquisition_offer_or_redemption,
	case when pbb."Margin Stock" = 0 then 'N' when pbb."Margin Stock" = 1 then 'Y' else null end as margin_stock,
	case when pbb."Subject to Withholding Tax" = 0 then 'N' when pbb."Subject to Withholding Tax" = 1 then 'Y' else null end as subject_to_withholding_tax,
	case when pbb."Defaulted Collateral Loan at Acquisition" = 0 then 'N' when pbb."Defaulted Collateral Loan at Acquisition" = 1 then 'Y' else null end as at_acquisition_defaulted_loan,
	case when pbb."Zero Coupon Obligation" = 0 then 'N' when pbb."Zero Coupon Obligation" = 1 then 'Y' else null end as zero_coupon_obligation,
	case when pbb."Covenant Lite" = 0 then 'N' when pbb."Covenant Lite" = 1 then 'Y' else null end as covenant_lite,
	case when pbb."Structured Finance Obligation / finance lease" = 0 then 'N' when pbb."Structured Finance Obligation / finance lease" = 1 then 'Y' else null end as structured_finance_obligation,
	case when pbb."Material Non-Credit Related Risk" = 0 then 'N' when pbb."Material Non-Credit Related Risk" = 1 then 'Y' else null end as material_non_credit_related_risk,
	case when pbb."Primarily Secured by Real Estate" = 0 then 'N' when pbb."Primarily Secured by Real Estate" = 1 then 'Y' else null end as primarily_secured_by_real_estate_or_loan,
	case when pbb."Interest Only Security" = 0 then 'N' when pbb."Interest Only Security" = 1 then 'Y' else null end as interest_only_security,
	case when pbb."Satisfies Other Criteria(1)" = 0 then 'N' when pbb."Satisfies Other Criteria(1)" = 1 then 'Y' else null end as satisfies_all_other_eligibility_criteria,
