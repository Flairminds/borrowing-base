...
class Obligor_Limit_Excess_GJ_GZ:
    ...
    def Obligor_No_GX_new(self):
        ...
        def process_row(row_index, loan_list_df):
            ...
            if is_matched(row_index, loan_list_df):
                ...
                lookup_index = lookup_array.index(loan_list_df.loc[row_index, "Obligor"])
                return loan_list_df["Obligor No GJ"][lookup_index]
    ...
    def Excess_HC_new(self):
        ...
        HC2 = max(0, loan_list_df["Obligor Loan Balance HA"][0:6].sum() - limit)
    ...
    def Limit(self):
        # RULE =IF(GL10>GL$6,0,IF(GL10<=GL$2,GN$2,GN$3))
        loan_list_df = self.file_df["Loan List"]
        No_of_Obligor_GL2 = self.No_of_Obligor_GL
        count_GL6 = (loan_list_df["Obligor Loan Balance GK"] > 0).sum()
        limit_dollar_GN2 = (
            self.total_Concentration_Test_Balance_OPB_Eligible_Unfunded
            * self.Limit_percent_GM2
        )
        limit_dollar_GN3 = (
            self.total_Concentration_Test_Balance_OPB_Eligible_Unfunded
            * self.Limit_percent_GM3
        )

        loan_list_df["Limit"] = loan_list_df.apply(
            lambda row: (
                0
                if row["Obligor Order"] > count_GL6
                else (
                    limit_dollar_GN2
                    if row["Obligor Order"] <= No_of_Obligor_GL2
                    else limit_dollar_GN3
                )
            ),
            axis=1,
        )
        ...
        def calculate_excess_fn(row, loan_list_df):
            ...
            if row["Obligor Industry FH Net Loan Balance"] == 0:
                ...
                lookup_value = loan_list_df.loc[
                    loan_list_df["Obligor No HD"] == row["Obligor Name"], "Haircut HE"
                ]
